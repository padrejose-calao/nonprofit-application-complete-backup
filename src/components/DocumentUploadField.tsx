...
const DocumentUploadField: React.FC<DocumentUploadFieldProps> = ({
  ...
  const handleDelete = async (document: DocumentInfo) => {
    ...
    const confirmed = await confirm({
      ...
      message: `Are you sure you want to delete "${document.name || 'this document'}"? This action cannot be undone.`,
      confirmText: 'Delete',
      variant: 'danger',
      onConfirm: async () => {
        try {
  ...
  const handleEdit = (document: DocumentInfo) => {
    ...
    setNewTags(document.tags && Array.isArray(document.tags) ? document.tags.join(', ') : '');
  };
              ...
              <div className="flex items-center space-x-3 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-4 text-xs text-gray-500">
                    ...
                    {enableVersioning && document.version && (
                      <span>v{document.version}</span>
                    )}
                  </div>
                  
                  {/* Tags */}
                  {showTags && document.tags && Array.isArray(document.tags) && document.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {document.tags.map((tag, index) => (
                        <span key={`${tag}-${index}`} className="px-2 py-1 bg-blue-100 text-xs text-blue-800 rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
